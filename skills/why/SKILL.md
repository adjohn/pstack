...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (recent edits and what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the MCP servers available in the session (`claude mcp list`, or the `mcp__*` tools visible in-session).
