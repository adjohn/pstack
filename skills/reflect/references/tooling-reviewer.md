...
- `Agent` (subagent) tool_use entries whose prompt names a skill path, and `Skill` tool invocations naming the skill (a skill invoked via the `Skill` tool never shows a `SKILL.md` Read)
