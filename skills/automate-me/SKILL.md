...
This skill orchestrates three others: an inline mining pass (see step 1), the `skill-creator` skill for authoring (from anthropics/skills, when installed; otherwise author the SKILL.md directly per step 4), and the **unslop** skill (prose discipline). It sequences them; it doesn't replace them.
...
Locate the active workspace's transcripts before fanning out. Claude Code stores them at `~/.claude/projects/<slug>/<session-uuid>.jsonl`, where `<slug>` is the absolute workspace path, symlinks resolved, with every character that is not a letter or digit (including `/`, `.`, and `_`) replaced by `-` (so `/Users/you/proj` becomes `-Users-you-proj`) and each line is one JSON message. Derive the directory from the current working directory; if the derived directory is missing, list `~/.claude/projects/` and pick the entry whose name matches your slugified cwd. Use only that path. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Shape: one or two questions with 2-4 options each (the tool's maximum is 4; fold overflow into a second question or rely on the built-in Other option), `multiSelect: true` for category questions. Start broad ("Which areas matter most?"), then follow up on selected areas with specific options. After the structured rounds, one free-form chat question catches anything the options missed.
...
Use the `skill-creator` skill (from anthropics/skills) to author the skill if it's installed; otherwise author the SKILL.md directly following the frontmatter rules below. Placement:
...
Apply the **unslop** skill and `skill-creator`'s writing guidelines (when installed) to every line. Both apply to any agent-read prose, not just skills.
...
- User wants a task-specific skill (not working conventions): `skill-creator` alone (or author the SKILL.md directly), no mining required.
...
- The `skill-creator` skill (from anthropics/skills), when installed: skill authoring process and writing guidelines.
