...
1. Capture a baseline trace via the project's verification skill (`verify-<app>`, created by `/create-verification-skill`).
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue model (default `haiku`). Review the diff. Capture a post-fix trace.
