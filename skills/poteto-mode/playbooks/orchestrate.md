...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the cloud agent's status in the Claude Code cloud dashboard (claude.ai/code). Transcript mtime is not liveness.
